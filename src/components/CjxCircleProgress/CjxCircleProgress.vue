...
export default {
  ...
  methods: {
    ...
    // 背景
    drawBgCircular() {
      const ctx = uni.createCanvasContext(this.bgCanvasId, this)
    ...
    drawCircular(startPercent) {
      ...
      if (!ctx) {
        ...
        const boxWidth = Number(this.boxWidth)
      ...
      const time = this.percent == 0 || this.duration < 50 ? 0 : this.duration / this.percent
      if (this.percent > 0) {
      ...
      if (this.percent == 0) {
        ...
        const eAngle = ((2 * Math.PI) / 100) * startPercent + this.sAngle
        this.drawArc(ctx, start, end)
      }
      setTimeout(() => {
